...
export async function createODRequest(data) {
  try {
    ...
    const currentCategoryCount = getStudentODValue(
      studentRecord,
      eventCategory,
    );
  ...
  try {
    ...
    if (role === "hod") {
      try {
        ...
        if (studentRecord) {
          const currentCount =
            studentRecord.od_count !== undefined &&
            studentRecord.od_count !== null
              ? studentRecord.od_count
              : 7;
          ...
          await Promise.all(
            teamRollNumbers.map(async (rollNo) => {
              try {
                ...
                if (teamMember) {
                  const memberCount =
                    teamMember.od_count !== undefined &&
                    teamMember.od_count !== null
                      ? teamMember.od_count
                      : 7;
