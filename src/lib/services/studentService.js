...
export async function getStudentByAppwriteUserId(appwriteUserId) {
  try {
    ...
    if (
      String(error?.message || "").includes("Attribute not found in schema")
    ) {
      return null;
    }
...
export async function createStudent(data) {
  const payload = {
    ...
    cgpa:
      data.cgpa !== undefined && data.cgpa !== "" && data.cgpa !== null
