...
const APPROVAL_STEPS = [
  {
    ...
    remarksField: "mentor_remarks",
    actionAtField: "mentor_action_at",
    pendingStatus: OD_STATUS.PENDING_MENTOR,
  },
  {
    key: "advisor",
    label: "Advisor",
    statusField: "advisor_status",
    remarksField: "advisor_remarks",
    actionAtField: "advisor_action_at",
    pendingStatus: OD_STATUS.PENDING_ADVISOR,
  },
  {
    key: "coordinator",
    label: "Coordinator",
    statusField: "coordinator_status",
    remarksField: "coordinator_remarks",
    actionAtField: "coordinator_action_at",
    pendingStatus: OD_STATUS.PENDING_COORDINATOR,
  },
  {
    key: "hod",
    label: "HOD",
    statusField: "hod_status",
    remarksField: "hod_remarks",
    actionAtField: "hod_action_at",
    pendingStatus: OD_STATUS.PENDING_HOD,
  },
];
...
export default function ODDetailsModal({ isOpen, onClose, odId }) {
  ...
  async function loadDetails() {
    try {
      ...
      const resolveSubmitterEmail = async (odRequestData) => {
        ...
        if (
          odRequestData.student_email &&
          odRequestData.student_email.includes("@")
        ) {
          return odRequestData.student_email;
        }
        if (
          odRequestData.student_id &&
          odRequestData.student_id.includes("@")
        ) {
          return odRequestData.student_id;
        }
        if (!odRequestData.student_id) return "";

        const dbUser = await getUserByAppwriteId(
          odRequestData.student_id,
        ).catch(() => null);
      ...
      const [logsData, eventData] = await Promise.all([
        ...
        data.event_id
          ? getEventById(data.event_id).catch(() => null)
      ...
      const userIdsToResolve = [
        ...new Set(
          logsDocs.map((log) => log.action_by_user_id).filter(Boolean),
        ),
      ];
      const logUsersMap = {};
      await Promise.all(
        userIdsToResolve.map(async (uid) => {
          ...
          if (!fac) {
            ...
            if (dbUser && dbUser.user_email) {
              fac = await getFacultyByEmail(dbUser.user_email).catch(
                () => null,
              );
            }
          }
          ...
          if (fac) {
            ...
            logUsersMap[uid] = {
              name: dbUser.user_name || dbUser.name || "Unknown",
              department: "",
            };
          } else {
      ...
      if (studentData?.advisor_id) {
        const docIdMatch = await getFacultyById(studentData.advisor_id).catch(
          () => null,
        );
        const facIdMatch = docIdMatch
          ? null
          : await getFacultyByFacultyId(studentData.advisor_id).catch(
              () => null,
            );
        const appwriteIdMatch =
          docIdMatch || facIdMatch
            ? null
            : await getFacultyByAppwriteId(studentData.advisor_id).catch(
                () => null,
              );

        const matchedFac = docIdMatch || facIdMatch || appwriteIdMatch;
        if (matchedFac) {
          resolvedAdvisorName = matchedFac.department
            ? `${matchedFac.name} (${matchedFac.department})`
            : matchedFac.name;
        } else {
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      addText("STUDENT INFORMATION", margin + 3, boxStartY + 6, {
        fontSize: 9,
        fontStyle: "bold",
      });
      drawLine(
      ...
      addText(
        studentDetails?.roll_no || studentDetails?.student_register_no,
        valueX,
        sy,
        { fontSize: 8 },
      );
      ...
      addText(
        `${studentDetails?.year || "N/A"} / ${studentDetails?.section || "N/A"}`,
        valueX,
        sy,
        { fontSize: 8 },
      );
      ...
      addText("EVENT INFORMATION", eventX + 3, boxStartY + 6, {
        fontSize: 9,
        fontStyle: "bold",
      });
      drawLine(
      ...
      addText(eventDetails?.event_name || odRequest?.event_id, eValueX, ey, {
        fontSize: 8,
        maxWidth: colWidth - 30,
      });
      ey += 10;
      ...
      addText(
        odRequest?.od_start_date
          ? new Date(odRequest.od_start_date).toLocaleDateString("en-IN")
          : "N/A",
        eValueX,
        ey,
        { fontSize: 8 },
      ...
      addText(
        odRequest?.od_end_date
          ? new Date(odRequest.od_end_date).toLocaleDateString("en-IN")
          : "N/A",
        eValueX,
        ey,
        { fontSize: 8 },
      ...
      const days = (() => {
        ...
        const s = new Date(odRequest.od_start_date);
        s.setHours(0, 0, 0, 0);
        const e = new Date(odRequest.od_end_date);
        e.setHours(0, 0, 0, 0);
        return Math.round((e - s) / (1000 * 60 * 60 * 24)) + 1;
      })();
      addText("Duration:", eLabelX, ey, { fontSize: 8, fontStyle: "bold" });
      addText(`${days} Day${days === 1 ? "" : "s"}`, eValueX, ey, {
        fontSize: 8,
      });
      ...
      addText("REASON FOR OUTWARD DUTY", margin + 3, y + 6, {
        fontSize: 9,
        fontStyle: "bold",
      });
      ...
      if (teamMembersData.length > 0) {
        ...
        addText(`TEAM MEMBERS (${teamMembersData.length})`, margin + 3, y + 6, {
          fontSize: 9,
          fontStyle: "bold",
        });
        drawLine(margin + 3, y + 8, margin + contentWidth - 3, y + 8, 0.2);
      ...
      checkPageBreak(50);
      addText("AUTHORIZATION / DIGITAL SIGNATURES", margin, y, {
        fontSize: 10,
        fontStyle: "bold",
      });
      y += 6;
      ...
      APPROVAL_STEPS.forEach((step, idx) => {
        ...
        const logForStep = logs.find(
          (l) => l.action_by_role === step.key && l.action === "approve",
        );
        const facName =
          logForStep && logUsers[logForStep.action_by_user_id]
      ...
      addText(footerText, pageWidth / 2, y, {
        fontSize: 6,
        color: [150, 150, 150],
        align: "center",
      });
      y += 3;
      addText(footerText2, pageWidth / 2, y, {
        fontSize: 6,
        color: [150, 150, 150],
        align: "center",
      });
      ...
      // Save
      pdf.save(
        `OD_Form_${odRequest?.student_roll_no || studentDetails?.roll_no || "Request"}_${odId?.slice(0, 8)}.pdf`,
      );
    } catch (error) {
      console.error("Error generating PDF:", error);
      alert(
        "Failed to download PDF. Please try again or use the Print option.",
      );
    } finally {
  ...
  const isGranted =
    odRequest?.current_status === OD_STATUS.GRANTED ||
    odRequest?.current_status === OD_STATUS.APPROVED;
  ...
  return (
    <>
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm print:hidden">
        <div className="animate-in fade-in zoom-in flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl duration-200">
          <div className="flex shrink-0 items-center justify-between border-b border-gray-100 p-4 sm:p-8">
            <div>
              ...
              <div className="space-y-8">
                ...
                {isRejected && (
                  <div className="rounded-2xl border border-red-200 bg-red-50 p-6 text-center">
                    ...
                    <h3 className="mb-1 text-xl font-black text-red-700">
                      OD Rejected
                    </h3>
                    <p className="text-sm text-red-600">
                      Your OD request was rejected. See details below.
                    </p>
                  </div>
                )}
                {isCancelled && (
                  <div className="rounded-2xl border border-gray-200 bg-gray-50 p-6 text-center">
                    ...
                    <h3 className="mb-1 text-xl font-black text-gray-700">
                      OD Cancelled
                    </h3>
                    <p className="text-sm text-gray-600">
                      This OD request was cancelled by the student.
                    </p>
                  </div>
                )}
                ...
                <div>
                  <h4 className="mb-6 text-[10px] font-black tracking-[0.2em] text-gray-400 uppercase">
                    Approval Progress
                  </h4>
                  <div className="overflow-x-auto">
                    <div className="relative min-w-[520px]">
                      {/* Progress line */}
                      <div className="absolute top-5 right-5 left-5 z-0 h-[2px] bg-gray-200"></div>
                            ...
                            >
                                ...
                                className={`flex h-10 w-10 items-center justify-center rounded-full border-2 transition-all duration-300 ${
                                  state === "approved"
                                    ...
                                    : state === "rejected"
                                      ...
                                      : state === "current"
                                        ? "animate-pulse border-[#1E2761] bg-white text-[#1E2761] shadow-lg shadow-[#1E2761]/20"
                              ...
                              {actionAt && (
                                <span className="mt-0.5 text-[9px] text-gray-400">
                                  {new Date(actionAt).toLocaleDateString([], {
                                    month: "short",
                                    day: "numeric",
                                  })}
                                </span>
                      ...
                      {isGranted && (
                        <div className="mt-4 text-center">
                          <span className="inline-flex items-center gap-1.5 rounded-full bg-green-100 px-4 py-1.5 text-xs font-black tracking-wider text-green-700 uppercase">
                            <svg
                ...
                <div className="rounded-2xl border border-blue-100 bg-blue-50/50 p-6">
                  <h4 className="mb-4 text-[10px] font-black tracking-[0.2em] text-blue-400 uppercase">
                    Student Details
                  </h4>
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-[#1E2761]">
                        {studentDetails?.roll_no ||
                          studentDetails?.student_register_no ||
                          "N/A"}
                      </p>
                    </div>
                    <div>
                      ...
                      <p className="text-sm font-bold break-all text-[#1E2761]">
                        {studentDetails?.email ||
                          submitterEmail ||
                          (odRequest?.student_id?.includes("@")
                ...
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 sm:gap-8">
                  <div>
                    ...
                    <p className="font-bold text-[#1E2761]">
                      {eventDetails?.event_name || odRequest.event_id || "N/A"}
                    </p>
                  </div>
                  <div>
                    <h4 className="mb-2 text-[10px] font-black tracking-[0.2em] text-gray-400 uppercase">
                    ...
                    <div className="flex items-center gap-2">
                      <p className="font-bold text-[#1E2761]">
                        {new Date(odRequest.od_start_date).toLocaleDateString()}{" "}
                        - {new Date(odRequest.od_end_date).toLocaleDateString()}
                      </p>
                      <span className="rounded bg-indigo-50 px-2 py-0.5 text-[10px] font-black tracking-widest text-indigo-700 uppercase">
                  ...
                  <div className="col-span-2">
                    ...
                    <p className="rounded-xl border border-gray-100 bg-gray-50 p-4 leading-relaxed text-gray-600">
                      {odRequest.reason}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="space-y-6">
                    {logs.length === 0 ? (
                      <p className="text-sm text-gray-400 italic">
                        No activity recorded yet. Waiting for mentor approval.
                      </p>
                    ) : (
                      logs.map((log, idx) => (
                        <div key={log.$id} className="relative flex gap-4">
                          {idx !== logs.length - 1 && (
                            <div className="absolute top-6 bottom-[-24px] left-[11px] w-[2px] bg-gray-100"></div>
                          )}
                          <div
                            className={`z-10 flex h-6 w-6 shrink-0 items-center justify-center rounded-full ${getLogActionMeta(log.action).iconClass}`}
                          >
                            {log.action === "approve" ? (
                              <svg
                                className="h-3 w-3"
                          ...
                          <div>
                            <div className="mb-1 flex items-center gap-2">
                              <span className="text-sm font-bold text-[#1E2761] capitalize">
                                {log.action_by_role}
                                {logUsers[log.action_by_user_id]?.name && (
                                  <span className="ml-1 font-medium text-gray-500">
                                    - {logUsers[log.action_by_user_id].name}
                                    {logUsers[log.action_by_user_id].department
                                      ? ` (${logUsers[log.action_by_user_id].department.toUpperCase()})`
                                      : ""}
                                  </span>
                                )}
                              </span>
                              <span
                                className={`text-[10px] font-black tracking-widest uppercase ${getLogActionMeta(log.action).badgeClass}`}
                              >
          ...
          <div className="flex shrink-0 justify-end gap-4 border-t border-gray-100 p-4 sm:p-8">
            {isGranted && (
              <>
                <button
                  onClick={handleDownloadPDF}
                  disabled={isDownloading}
                  className="flex items-center gap-2 rounded-xl bg-indigo-600 px-6 py-3 font-bold text-white transition-all hover:bg-indigo-700 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {isDownloading ? (
                ...
                <button
                  ...
                  className="flex items-center gap-2 rounded-xl bg-green-600 px-6 py-3 font-bold text-white transition-all hover:bg-green-700 active:scale-95"
                >
                  <svg
                    className="h-5 w-5"
                    fill="none"
            ...
            <button
              onClick={onClose}
              className="rounded-xl bg-[#1E2761] px-6 py-3 font-bold text-white transition-all hover:bg-[#2d3a7d] active:scale-95"
            >
              Close Details
            </button>
          </div>
      ...
      {isGranted && odRequest && (
        <div
          ref={printRef}
          className="print-container fixed inset-0 z-[999999] hidden h-full w-full bg-white p-12 text-black print:block"
        >
          <style
          ...
          <div className="mb-8 grid grid-cols-2 gap-8 text-sm">
            ...
            <div className="rounded border border-black p-5">
              <h3 className="mb-4 border-b border-black pb-2 text-lg font-bold tracking-wide uppercase">
                Student Information
              </h3>
              <div className="space-y-3">
                ...
                <p>
                  <span className="font-bold">Year / Sec:</span>{" "}
                  {studentDetails?.year} / {studentDetails?.section}
                </p>
                <p>
                  <span className="font-bold">Advisor:</span> {advisorName}
                </p>
              </div>
            </div>
            ...
            <div className="rounded border border-black p-5">
              ...
              <div className="space-y-3">
                ...
                <p>
                  <span className="font-bold">Duration:</span> {odDays} Days
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="mb-8 rounded border border-black p-5">
            <h3 className="mb-3 text-lg font-bold tracking-wide text-gray-800 uppercase">
              Reason for Outward Duty
            </h3>
          ...
          <h3 className="mb-4 text-lg font-bold tracking-wide text-gray-800 uppercase">
            Authorization / Digital Signatures
          </h3>
          <div className="grid grid-cols-4 gap-4 text-center">
            {APPROVAL_STEPS.map((step) => {
              ...
              return (
                <div
                  key={step.key}
                  className="flex min-h-[140px] flex-col items-center justify-between rounded border border-black bg-gray-50/20 p-3"
                >
                  <div className="mb-2 w-full border-b border-gray-300 pb-1 text-[10px] font-black text-[#1E2761] uppercase">
                    {step.label}
                  </div>
                  {state === "approved" ? (
                    <>
                      <div className="mb-1 text-xs font-bold text-green-700 uppercase">
                        APPROVED
                      </div>
                      <div className="mb-1 text-[10px] leading-tight font-bold text-gray-900">
                        {facName || "Verified Approver"}
                      </div>
                    ...
                    <div className="text-xs font-medium text-gray-300 italic">
                      Awaiting
                    </div>
                  )}
                </div>
          ...
          <div className="mt-12 border-t border-gray-200 pt-6 text-center text-[10px] font-medium text-gray-400">
            This is a digitally generated document via Sri Eshwar Student
            Participation Tracking System (SPTS). No physical signature is
            required.
          </div>
        </div>
      )}
