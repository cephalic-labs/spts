...
export default function SignInModal({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div ref={modalRef} className="relative w-full max-w-md mx-4 bg-[var(--card-bg)] rounded-[2.5rem] shadow-2xl border border-[var(--card-border)] dark:border-white/5 overflow-hidden z-10 animate-scaleIn">
        <button onClick={onClose} className="absolute top-6 right-6 z-20 w-10 h-10 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center transition-all duration-300 hover:scale-110 hover:rotate-90" aria-label="Close">
        ...
        <div className="bg-primary p-12 text-center relative overflow-hidden">
          ...
          <p className="text-white/80 text-lg">Sign in to your SPTS account</p>
        </div>

        <div className="p-12">
          <button onClick={handleGoogleSignIn} className="w-full flex items-center justify-center gap-4 bg-[var(--bg-color)] hover:bg-cool-gray/20 dark:hover:bg-white/5 text-[var(--text-color)] font-black text-lg py-5 px-8 rounded-2xl border-2 border-[var(--card-border)] dark:border-white/10 transition-all duration-300 shadow-sm hover:shadow-xl group active:scale-95">
